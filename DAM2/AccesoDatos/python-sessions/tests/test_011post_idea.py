...
{
 "model": "idearest04app.customuser",
 "pk": 1,
 "fields": {
...
{
 "model": "idearest04app.usersession",
 "pk": 1,
 "fields": {
  "creator": 1,
  "token": "''' + self.random_token + '''"
    ...
    def test_new_idea_unauthenticated_response(self):
        def test_body():
            expected = {"error": "Not valid token or missing header"}
            client_body = {
    ...
    def test_missing_parameter_response(self):
        def test_body():
            expected = {"error": "Missing parameter in body"}
            client_body = {
                "new_idea_title": "ATitle",
            }
    ...
    def test_category_not_valid_response(self):
        def test_body():
            expected = {"error": "Category was not found"}
            client_body = {
                "new_idea_title": "ATitle",
                "content": "AContent"
            }
    ...
    def __assert_test_database_contains(self, string_sqlite_tuple):
        command = 'sqlite3.exe apis/IdeAPI/db.test.sqlite3 "SELECT id, title, description FROM idearest04app_idea"'
