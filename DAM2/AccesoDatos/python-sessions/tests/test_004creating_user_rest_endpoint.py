...
class IdeAPIRegisterHappyPathTestCase(unittest.TestCase):
    ...
    def test_unsupported(self):
        def test_body():
            self.__assert_get_endpoint("v1/users", {"error": "HTTP method not supported"}, 405)
    ...
    def test_user_added_to_db(self):
        ...
        command = 'sqlite3.exe apis/IdeAPI/db.sqlite3 "SELECT COUNT(email) FROM idearest04app_customuser WHERE email=\'' + random_email + '\'"'
    ...
    def test_user_added_to_db_with_correctly_hashed_pw(self):
        ...
        # And now verify DB
        self.assertTrue(os.path.isdir(context.IDEAPI_ROOT + "idearest04app"))
        command = 'sqlite3.exe apis/IdeAPI/db.sqlite3 "SELECT encrypted_password FROM idearest04app_customuser WHERE email=\'' + random_email + '\'"'
        output = subprocess.Popen(command, shell=True, cwd=context.PROJECT_ROOT, stdout=subprocess.PIPE).communicate()[0]
