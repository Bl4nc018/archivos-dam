...
class IdeAPICategoryIdeaCommentModelsTestCase(unittest.TestCase):
    ...
    def test_categories_were_added(self):
        ...
        command = 'sqlite3.exe apis/IdeAPI/db.sqlite3 "SELECT COUNT(*) FROM idearest04app_category  WHERE title=\'Sociedad\'"'
        ...
        command = 'sqlite3.exe apis/IdeAPI/db.sqlite3 "SELECT COUNT(*) FROM idearest04app_category  WHERE title=\'Educación\'"'
        output = subprocess.Popen(command, shell=True, cwd=context.PROJECT_ROOT, stdout=subprocess.PIPE).communicate()[0]
        self.assertEqual(output.decode('utf8').strip(), "1")
        command = 'sqlite3.exe apis/IdeAPI/db.sqlite3 "SELECT COUNT(*) FROM idearest04app_category  WHERE title=\'Arte\'"'
        output = subprocess.Popen(command, shell=True, cwd=context.PROJECT_ROOT, stdout=subprocess.PIPE).communicate()[0]
        self.assertEqual(output.decode('utf8').strip(), "1")
        command = 'sqlite3.exe apis/IdeAPI/db.sqlite3 "SELECT COUNT(*) FROM idearest04app_category  WHERE title=\'Invenciones\'"'
        output = subprocess.Popen(command, shell=True, cwd=context.PROJECT_ROOT, stdout=subprocess.PIPE).communicate()[0]
        self.assertEqual(output.decode('utf8').strip(), "1")

    def test_category_idea_comment_lines(self):
        models_filename = context.IDEAPI_ROOT + "idearest04app/models.py"
    ...
    def __expected_lines_content(self):
        ...
        return [
            ...
            {"line": "models.CharField(max_length=2000)", "found": False},
