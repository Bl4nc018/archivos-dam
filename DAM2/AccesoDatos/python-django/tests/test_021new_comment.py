...
class WallAPINewCommentEndpointTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        file = open("test_021new_comment.json", "w")
        file.write('[{"model": "wallrest04app.entry", "pk": 1, "fields": {"title": "Title1", "content": "Content1", "publication_date": "2022-07-13T10:58:27Z"}}, {"model": "wallrest04app.entry", "pk": 2, "fields": {"title": "Title2", "content": "Content2", "publication_date": "2022-07-13T11:04:50Z"}}]')
        file.close()
        ...
        modified_settings_file = open(context.WALL_API_ROOT + "WallAPI/settings_test_021new_comment.py", "w")
    ...
    def test_new_comment_response(self):
        def test_body():
            expected = {"new_comment_saved": True}
    ...
    def test_new_comment_is_actually_added(self):
        def test_body():
            ...
            self.__assert_test_database_contains("SELECT id, content FROM wallrest04app_comment", "|AContent2")
        # Run server and run test
        self.__test_with_server_alive(test_body)
    ...
    def test_new_comment_entry2_is_actually_added(self):
        def test_body():
            ...
            # And assert content in DB
            self.__assert_test_database_contains("SELECT id, content FROM wallrest04app_comment WHERE entry_id = 2", "|AContent3")
        # Run server and run test
        self.__test_with_server_alive(test_body)
    ...
    def __test_with_server_alive(self, block):
        def run_server():
            proc = subprocess.Popen("python manage.py testserver ../../tests/test_021new_comment.json --noinput --settings WallAPI.settings_test_021new_comment", creationflags=subprocess.CREATE_NEW_PROCESS_GROUP, cwd=context.WALL_API_ROOT)
            time.sleep(4)
        ...
        def app_dir_exists():
            return os.path.isdir(context.WALL_API_ROOT + "wallrest04app")
    ...
    def tearDown(self):
        temp_settings = os.path.abspath(context.WALL_API_ROOT + "WallAPI/settings_test_021new_comment.py")
