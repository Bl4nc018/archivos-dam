...
class DashboardsAPINewQuestionEndpointTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        file = open("test_027questions_post.json", "w")
        file.write('''
                [
          {
            "model": "dashboard04app.dashboard",
            "pk": 1,
            "fields": {
              "title": "Forocoches",
              "content": "Test"
        ...
        modified_settings_file = open(context.DASHBOARD_API_ROOT + "DashboardAPI/settings_test_027questions_post.py", "w")
    ...
    def test_new_question_response(self):
        def test_body():
            client_body = {"title": "New Question", "content": "A Question"}
    ...
    def test_new_question_is_actually_adde1(self):
        def test_body():
            ...
            self.__assert_test_database_contains("SELECT id, title, content FROM dashboard04app_question WHERE dashboard_id=1", "|New Question|A Question")
        # Run server and run test
        self.__test_with_server_alive(test_body)

    def test_new_question_is_actually_added2(self):
        def test_body():
            client_body = {"title": "New Question2", "content": "A Question2"}
            # Send POST
            self.__assert_endpoint("api/v1/dashboards/2/questions", client_json=client_body, expected_status_code=201)
            # And assert content in DB
            self.__assert_test_database_contains("SELECT id, title, content FROM dashboard04app_question WHERE dashboard_id=2", "|New Question2|A Question2")
        # Run server and run test
        self.__test_with_server_alive(test_body)
    ...
    def __test_with_server_alive(self, block):
        def run_server():
            proc = subprocess.Popen("python manage.py testserver ../../tests/test_027questions_post.json --noinput --settings DashboardAPI.settings_test_027questions_post", creationflags=subprocess.CREATE_NEW_PROCESS_GROUP, cwd=context.DASHBOARD_API_ROOT)
            time.sleep(4)
        ...
        def app_dir_exists():
            return os.path.isdir(context.DASHBOARD_API_ROOT + "dashboard04app")
    ...
    def tearDown(self):
        temp_settings = os.path.abspath(context.DASHBOARD_API_ROOT + "DashboardAPI/settings_test_027questions_post.py")
