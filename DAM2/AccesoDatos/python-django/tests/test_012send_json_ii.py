...
class SimpleAPIPostFavoriteAnimalEndpointTestCase(unittest.TestCase):
    ...
    def test_animal_method_not_allowed_error_response(self):
        def test_body():
            self.__assert_endpoint("preferred_animal", client_json=None, expected_json_content=None, expected_status_code=405)
        # Run server and run test
        self.__test_with_server_alive(test_body)

    def test_animal_bad_request_wrong_body_error_response(self):
        def test_body():
            self.__assert_endpoint("preferred_animal", client_json={"wrong": "Haha"}, expected_status_code=400)
        # Run server and run test
        self.__test_with_server_alive(test_body)

    def test_animal_bad_request_missing_body_error_response(self):
        def test_body():
            response = requests.post("http://localhost:8000/preferred_animal")
    ...
    def test_animal_cat_response(self):
        def test_body():
            self.__assert_endpoint("preferred_animal", client_json={"name": "Cat"}, expected_json_content={"message": "Great! Seven lives will be enough"}, expected_status_code=200)
        # Run server and run test
        self.__test_with_server_alive(test_body)
    ...
    def test_animal_other_response(self):
        def test_body():
            ...
            self.__assert_endpoint("preferred_animal", client_json={"name": r}, expected_json_content={"message": "OK! Have a nice day"}, expected_status_code=200)
        # Run server and run test
        self.__test_with_server_alive(test_body)
    ...
    def __test_with_server_alive(self, block):
        ...
        def app_dir_exists():
            return os.path.isdir(context.SIMPLE_API_ROOT + "simplerest04app")
