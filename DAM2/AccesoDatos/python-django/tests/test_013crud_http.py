...
class SimpleAPICRUDExampleEndpointTestCase(unittest.TestCase):
    ...
    def test_animal_method_not_allowed_error_response(self):
        def test_body_get():
            response = requests.get("http://localhost:8000/http_methods")
            self.assertEqual(response.json()["message"], "Reading some data, huh?")

        def test_body_post():
            response = requests.post("http://localhost:8000/http_methods")
            self.assertEqual(response.json()["message"], "This should create a new thing")

        def test_body_put():
            response = requests.put("http://localhost:8000/http_methods")
            self.assertEqual(response.json()["message"], "You can update any element with this")

        def test_body_delete():
            response = requests.delete("http://localhost:8000/http_methods")
            self.assertEqual(response.json()["message"], "This will remove one or many elements")
