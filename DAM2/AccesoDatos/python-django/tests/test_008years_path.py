...
class SimpleAPIYearsPathEndpointTestCase(unittest.TestCase):
    ...
    def test_some_years_in_past(self):
        def test_body():
            ...
            year = current - r
            self.__assert_endpoint("years_since/" + str(year), {"years_passed": r})
    ...
    def test_current_years(self):
        def test_body():
            current = datetime.now().date().year
            self.__assert_endpoint("years_since/" + str(current), {"years_passed": 0})
        # Run server and run test
        self.__test_with_server_alive(test_body)
    ...
    def test_year_in_the_future_error_response(self):
        def test_body():
            current = datetime.now().date().year
            self.__assert_endpoint("years_since/" + str(current+1), {"error": "Years in the future are not supported"}, 400)
        # Run server and run test
        self.__test_with_server_alive(test_body)
