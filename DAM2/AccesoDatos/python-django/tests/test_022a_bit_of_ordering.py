...
class WallAPIOrderingEndpointTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        file = open("test_022a_bit_of_ordering.json", "w")
        file.write('[{"model": "wallrest04app.entry", "pk": 1, "fields": {"title": "Title1", "content": "Content1", "publication_date": "2022-07-13T10:58:27Z"}}, {"model": "wallrest04app.entry", "pk": 2, "fields": {"title": "Title2", "content": "'+self.r1+'", "publication_date": "2022-07-13T11:04:50Z"}}]')
        file.close()
    ...
    def __test_with_server_alive(self, block):
        def run_server():
            proc = subprocess.Popen("python manage.py testserver ../../tests/test_022a_bit_of_ordering.json --noinput", creationflags=subprocess.CREATE_NEW_PROCESS_GROUP, cwd=context.WALL_API_ROOT)
            time.sleep(4)
