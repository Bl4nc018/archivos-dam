...
class SimpleAPIPrimesQueryEndpointTestCase(unittest.TestCase):
    ...
    def test_zero_returns_error(self):
        def test_body():
            self.__assert_endpoint("prime?i=0", {"error": "Parameter must be a number greater than zero"}, 400)
        # Run server and run test
        self.__test_with_server_alive(test_body)

    def test_missing_parameter_returns_error(self):
        def test_body():
            self.__assert_endpoint("prime", {"error": "Missing required 'i' parameter"}, 400)
        # Run server and run test
        self.__test_with_server_alive(test_body)

    def test_1_correct_result(self):
        def test_body():
            self.__assert_endpoint("prime?i=1", {"is_prime_number": True})
        # Run server and run test
        self.__test_with_server_alive(test_body)

    def test_2_correct_result(self):
        def test_body():
            self.__assert_endpoint("prime?i=2", {"is_prime_number": True})
        # Run server and run test
        self.__test_with_server_alive(test_body)

    def test_4_correct_result(self):
        def test_body():
            self.__assert_endpoint("prime?i=4", {"is_prime_number": False})
        # Run server and run test
        self.__test_with_server_alive(test_body)
    ...
    def test_small_random_has_correct_result(self):
        def test_body():
            ...
            self.__assert_endpoint("prime?i=" + str(r), {"is_prime_number": self.__is_p(r)})
        # Run server and run test
        self.__test_with_server_alive(test_body)
