...
class DashboardsAPINewAnswerEndpointTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        file = open("test_029answers_post.json", "w")
        file.write('''
                [
          {
        ...
        modified_settings_file = open(context.DASHBOARD_API_ROOT + "DashboardAPI/settings_test_029answers_post.py", "w")
    ...
    def test_new_answer_is_actually_added1(self):
        def test_body():
            client_body = {"content": "An Answer2"}
            # Send POST
            self.__assert_endpoint("api/v1/questions/1/answers", client_json=client_body, expected_status_code=201)
            # And assert content in DB
            self.__assert_test_database_contains("SELECT id, content FROM dashboard04app_answer WHERE question_id=1", "|An Answer2")
        # Run server and run test
        self.__test_with_server_alive(test_body)

    def test_new_answer_is_actually_added2(self):
        def test_body():
            client_body = {"content": "An Answer3"}
            # Send POST
            self.__assert_endpoint("api/v1/questions/2/answers", client_json=client_body, expected_status_code=201)
            # And assert content in DB
            self.__assert_test_database_contains("SELECT id, content FROM dashboard04app_answer WHERE question_id=2", "|An Answer3")
        # Run server and run test
        self.__test_with_server_alive(test_body)
    ...
    def __test_with_server_alive(self, block):
        def run_server():
            proc = subprocess.Popen("python manage.py testserver ../../tests/test_029answers_post.json --noinput --settings DashboardAPI.settings_test_029answers_post", creationflags=subprocess.CREATE_NEW_PROCESS_GROUP, cwd=context.DASHBOARD_API_ROOT)
            time.sleep(4)
    ...
    def tearDown(self):
        temp_settings = os.path.abspath(context.DASHBOARD_API_ROOT + "DashboardAPI/settings_test_029answers_post.py")
