...
class DashboardsAPIAllEndpointTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        file = open("test_025dashboards_intro.json", "w")
        file.write('''
        [
  {
  ...
  },
  {
    ...
    def __test_with_server_alive(self, block):
        def run_server():
            proc = subprocess.Popen("python manage.py testserver ../../tests/test_025dashboards_intro.json --noinput", creationflags=subprocess.CREATE_NEW_PROCESS_GROUP, cwd=context.DASHBOARD_API_ROOT)
            time.sleep(4)
