...
def entry_comments(request, path_param_id):
    if request.method == "GET":
        comments = Comment.objects.filter(entry_id=path_param_id)
        json_response = []
        for row in comments:
            json_response.append(row.to_json())
        return JsonResponse(json_response, safe=False)
        ...
        # Creamos una nueva instancia de Comment. Nótese pasamos entry_id=... en vez de entry=... para indicar que
        # nos referimos a la clave primaria (id) del atributo. Esto se hace así para los models.ForeignKey
