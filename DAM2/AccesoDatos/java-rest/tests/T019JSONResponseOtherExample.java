...
public class T019JSONResponseOtherExample {
    ...
    public void testExampleEndpointCharacterDescription() {
        try {
            ...
            JSONObject result = getUrl("http://localhost:8104/example4");
            assert(result != null);
            assertEquals("Lisa Simpson es la pequeña saxofonista que adora el Jazz y a los animales", result.getString("characterDescription"));
    ...
    public void testExampleEndpointCharacterPopularity() {
        try {
            ...
            JSONObject result = getUrl("http://localhost:8104/example4");
            assert(result != null);
            assertEquals(10, result.getInt("characterPopularity"));
            this.stopServer.invoke(this.currentServerInstance);
        } catch (ClassNotFoundException e) {
            fail("La clase especificada no existe");
        } catch (NoSuchMethodException e) {
