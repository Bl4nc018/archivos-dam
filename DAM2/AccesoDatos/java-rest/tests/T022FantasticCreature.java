...
public class T022FantasticCreature {
    ...
    public void testName() {
        try {
            ...
            JSONObject result = getUrl("http://localhost:8104/creature");
            assert(result != null);
            assertEquals("Duende rojo del volcán", result.getString("name"));
    ...
    public void testLifespan() {
        try {
            ...
            JSONObject result = getUrl("http://localhost:8104/creature");
            assert(result != null);
            assertEquals(342, result.getInt("lifespanYears"));
            this.stopServer.invoke(this.currentServerInstance);
        } catch (ClassNotFoundException e) {
            fail("La clase especificada no existe");
        } catch (NoSuchMethodException e) {
    ...
    public void testDangerous() {
        try {
            ...
            JSONObject result = getUrl("http://localhost:8104/creature");
            assert(result != null);
            assertEquals(true, result.getBoolean("dangerous"));
            this.stopServer.invoke(this.currentServerInstance);
        } catch (ClassNotFoundException e) {
            fail("La clase especificada no existe");
        } catch (NoSuchMethodException e) {
    ...
    public void testMagicPower() {
        try {
            ...
            assert(magic != null);
            assertEquals(42, magic.getInt("power"));
            this.stopServer.invoke(this.currentServerInstance);
        } catch (ClassNotFoundException e) {
            fail("La clase especificada no existe");
        } catch (NoSuchMethodException e) {
    ...
    public void testAbility() {
        try {
            ...
            JSONObject magic = result.getJSONObject("magic");
            assert(magic != null);
            assertEquals("Puede pulverizar a sus enemigos con una llamarada de fuego", magic.getString("ability"));
            this.stopServer.invoke(this.currentServerInstance);
        } catch (ClassNotFoundException e) {
            fail("La clase especificada no existe");
        } catch (NoSuchMethodException e) {
