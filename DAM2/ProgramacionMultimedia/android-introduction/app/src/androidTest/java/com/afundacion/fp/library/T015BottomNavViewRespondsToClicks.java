...
public class T015BottomNavViewRespondsToClicks {
    ...
    public void checkToast1IsShown() {
        ...
        Espresso.onView(withText("Este es el primer Digimon")).inRoot(isOverlayWindow()).check(matches(isDisplayed()));
    }

    @Test
    public void checkToast2IsShown() {
        Espresso.onView(withBottomMenuItem("item2")).perform(click());
        Espresso.onView(withText("Clicaste Digimon 2")).inRoot(isOverlayWindow()).check(matches(isDisplayed()));
    }

    @Test
    public void checkToast3IsShown() {
        Espresso.onView(withBottomMenuItem("item3")).perform(click());
        Espresso.onView(withText("Has clicado en Digimon 3")).inRoot(isOverlayWindow()).check(matches(isDisplayed()));
    }
