...
ruta_archivo = r"C:\Users\joker\OneDrive\Desktop\Curso 23-24\Desenvolvemento de interfaces\Primer trimestre\ahorcado.txt"
...
while repetir == "s":
    ...
    dificultad = input("Introduzca el nivel de dificultad del juego ('Facil', 'Normal', 'Dificil'): ")
    ...
    if dificultad in lvls:
        ...
        if palabra_oculta == palabra_aleatoria:
            print("\n")
            print(f"¡Felicidades! Has adivinado la palabra correctamente. La palabra es: {palabra_aleatoria}")
        else:
            print("\n")
            print(f"Se acabaron los intentos. La palabra era: {palabra_aleatoria}. ¡Perdiste!")
    else:
        print("\nDificultad no válida. Por favor, elija entre 'Facil', 'Normal' o 'Dificil'.")
